...
describe Guide do
  ...
  describe '#launch!' do
    ...
    it 'outputs a introductory message' do
      ...
      expect{ subject.launch! }.to output(/Welcome to the Food Finder/).to_stdout
    end
  ...
  describe 'performing actions' do
    ...
    context 'with invalid action' do
      ...
      it 'outputs list of valid actions' do
        ...
        expect(subject).to receive(:output_valid_actions).twice
    ...
    context 'with find action' do
      ...
      it 'finds restaurants with matching name keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to match(/Cafe.+Indian.+\$30.00/)
        expect(lines[13]).to eq("-" * 60)
      end
      ...
      it 'finds restaurants with matching cuisine keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to match(/\A\sHot Tamale.+Mexican.+\$25.00\z/)
        expect(lines[13]).to eq("-" * 60)
      end
      ...
      it 'finds restaurants with prices less than keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to match(/\A\sPita Pocket.+Fast Food.+\$10.00\z/)
        expect(lines[13]).to match(/\A\sQuick Cup.+Coffee.+\$5.00\z/)
        expect(lines[14]).to match(/\A\sTaste Of Little Italy.+Pizza.+\$10.00\z/)
